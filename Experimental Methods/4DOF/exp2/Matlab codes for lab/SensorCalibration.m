...
FileName=['Cal12_',num2str(Answer)];
...
%% save data
if ischar(FileName)
    save(FileName, 'data','time','A','Fs','F','t0');
end
...
subplot 212
p = polyfit(Vmass,laser,3);

switch Answer
    case 1
        Cm1 = fliplr(p);save('Cm1','Cm1');
    case 2
        Cm2 = fliplr(p);save('Cm2','Cm2');
    case 3
        Cm3 = fliplr(p);save('Cm3','Cm3');
    case 4
        Cm4 = fliplr(p);save('Cm4','Cm4');
        GenerateC_forStepSine;
end

plot(Vmass,polyval(p,Vmass),'o',Vmass,laser,'-r');
ylabel('Displacement [mm]');
xlabel('V_m_1 [V] (PSD)');
legend({'V_m_1','fitted curve'},'Location','NorthWest');
title(['Displacement=',...
    num2str(p(1),'%.3f'),'*x^3+',num2str(p(2),'%.3f'),'*x^2+',...
    num2str(p(3),'%.3f'),'*x+',num2str(p(4),'%.3f')]);

if Answer==4
    ANS=questdlg('Check calibration?');
    if strcmp(ANS,'Yes'),SinInput;end;
end